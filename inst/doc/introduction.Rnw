...
%\VignetteIndexEntry{An Introduction to the Data Merging GUI}
...
In some situations, we have several files containing the same subjects
...
This part allows the user to match the data from the different experimental
...
In the preferences tab, the user can choose whether the y-scales are
free for different data files when drawing the plots facetted by the
files; whether the numerical p-values or the flag symbols are displayed
in the summary tab or not. All the changes will be presented in the
Summary tab.
...
in the right pane. Here the variables Grade and Score have been selected
...
of the variable, facetted by file. If more than two variables are
selected, then only the first two numeric variables are used to draw
a scatterplot instead of a histogram or a barchart for individual
variable.
...
symbols correspond to the p-values which are less than 0.05. The column
`Unit' gives the misclassification rates for each variable if it is
used to seperate the files by using classification tree (the rpart
package). The column `Dist' compute the p-values of the Kolmogorov-Smirnov
tests between different sources for each variable. `Miss' gives the
p-values of the Chi-square tests for the counts of missing and non-missing
values between different files for each variable.
